...
- **`services/`** – integrations with external APIs like Jellyfin, Navidrome, Last.fm, OpenAI and GetSongBPM. Planned metadata work also adds MusicBrainz for original-release resolution and ListenBrainz for tag enrichment.
...
2. External lookups (media server, Last.fm, GetSongBPM and OpenAI) are performed asynchronously using `httpx`. Planned metadata enrichment adds MusicBrainz and ListenBrainz as additional optional sources.
